...
#endif
/* set the FQDN here */
	shp = NULL; //JT gethostbyname(hostname);
	if (shp != NULL) {
	...
	if (getsockname(0, (struct sockaddr *) &virtual_addr, &virtual_len) == 0) {
	    ...
	    shp = NULL;//JT gethostbyaddr((char *) &virtual_ptr->sin_addr, sizeof(struct in_addr), AF_INET);
	    if (shp != NULL) {
...
/* H* fix: if we're logged in at all, we can't log in again. */
	...
	*auth++ = 0;
	...
	if (checkuser("ftp") || checkuser("anonymous")) {
	    ...
	    if (hostname[0]
		&& ((tmphostent = NULL /*JT gethostbyname(hostname)*/))) {
		...
		for (machinecount = 0;
		  entry->arg[machinecount] && (entry->arg[machinecount])[0];
		    ...
		    if ((tmphostent = NULL /*JT gethostbyname(entry->arg[machinecount])*/)) {
			/*
...
void dolog(struct sockaddr_in *sin)
{
     ...
     */
  dns_again:
    hp = NULL; //JT gethostbyaddr((char *) &sin->sin_addr, sizeof(struct in_addr), AF_INET);
